...
app.post("/api/login", async (req, res) => {
  ...
  try {
    const [rows] = await
    conn.execute("SELECT * FROM Accounts WHERE User_name = ?", [username]);
    const user = rows[0];
...
app.post("/api/register", async (req, res) => {
  ...
  try {
    ...
    const insertUserSql =
      "INSERT INTO Accounts (User_name, Password) VALUES (?, ?)";
...
app.post("/api/submit-form", async (req, res) => {
  try {
    ...
    const [results] =
      await conn.query("INSERT INTO submissions SET ?", formData);
    res.status(200).json(
        {message: "Form data submitted successfully", data: results});
  } catch (error) {
    console.error(error);
    res.status(500).json({error: error.message});
  }
